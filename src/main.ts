...
const getRequiredArg = <K extends keyof Args>(
  name: K,
): NonNullable<Args[K]> => {
...
const buildPackageConfig = async (remote?: Remote): Promise<SyncConfig> => {
  const workDir = `${DOMAIN_INSTANCE.getRootDir(ARG_ENV)}/${ARG_ENV}/${ARG_DOMAIN_NAME}`;
  const runtimeDir = `${DOMAIN_INSTANCE.getRootDir(ARG_ENV)}/${ARG_ENV}/_runtime/${ARG_DOMAIN_NAME}`;
  return {
    //
    ...
    templateExcludeRegex:
      (await DOMAIN_INSTANCE.getTemplateExcludeRegex(ARG_ENV)) || undefined,
    packageExcludeRegex:
      (await DOMAIN_INSTANCE.getPackageExcludeRegex(ARG_ENV)) || undefined,
    ...
    unpackHook:
      remote &&
      (targetDir =>
        DOMAIN_INSTANCE.unpackHook(ARG_ENV, remote, targetDir, runtimeDir)),
    completeHook:
      remote &&
      (targetDir =>
        DOMAIN_INSTANCE.completeHook(ARG_ENV, remote, targetDir, runtimeDir)),
...
(async () => {
  ...
  const success = await remoteSession(config, async remote => {
    ...
    if (ARGS["--deploy"]) {
      await syncPackage(
        remote,
        getRequiredArg("--domain"),
        await buildPackageConfig(remote),
      );
      return;
    }
